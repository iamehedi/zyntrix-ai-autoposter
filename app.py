...
def generate_and_validate_content(topic, history_summary, provider="groq") -> dict:
    ...
    creator = Agent(
        ...
        backstory=(
            ...
            "LANGUAGE: natural conversational Bangla — ALWAYS written in proper Bengali Unicode "
            "script (বাংলা লিপি), NEVER in Romanized Bangla/Banglish. CRITICAL: every Bengali word "
            "must be typed in Bengali script (e.g. write 'সবচেয়ে বড়', NOT 'shobcheye boro'). "
            "Only common tech terms stay in English (Website, App, API, UI/UX, SEO, CRM, Database, "
            "Cloud, etc.) — never force awkward Bengali translations of those.\n"
            ...
            "ENDING RULES — the post must teach, explain or entertain, then STOP naturally. "
            "NEVER end with: a promotional or sales line about Zyntrix ('আমরা এমন সলিউশন বানাই...', "
            "'Zyntrix helps your business...'), a CTA ('contact us', 'comment below', 'DM us', 'follow us', "
            "'save this'), a motivational/inspirational filler ('your business deserves better', 'the future "
            "is AI', 'এটাই ভবিষ্যৎ', 'now start today'), or any sentence whose only job is to sound "
            "inspirational or professional. The ending must complete the actual topic: a practical takeaway, "
            "a funny observation, a relatable conclusion, a small insight, a natural question, a punchline, "
            "a short comparison, or a useful warning — 1-3 sentences, never a separate 'Conclusion' section, "
            "never an essay-style summary, never a forced joke unrelated to the topic. No generic "
            "engagement questions just to get comments. A post does NOT need a CTA, a question, a "
            "motivational line, or a brand mention to be complete. Mention 'Zyntrix' ONLY when it naturally "
            "belongs in the explanation, example, experiment, workflow or story — if it doesn't, leave the "
            "post brand-free. Vary the ending across posts. If the educational point is already complete, "
            "STOP — delete any paragraph that only repeats, promotes, motivates or pads.\n"
            ...
            "accurate? non-promotional? different from previous posts? readable as text-only? ENDING CHECK: "
            "does the last paragraph promote Zyntrix, use a forced CTA, sound like generic AI motivation, "
            "or exist only to pad the post? If the post feels complete without the last sentence, delete it. "
            "Assign honest scores (usefulness>=8, uniqueness>=8, human_feel>=8, technical_accuracy>=9, "
    ...
    creator_task = Task(
        description=(
            ...
            "Write a complete text-only Facebook post in natural conversational Bangla — in proper "
            "Bengali Unicode script (বাংলা লিপি), NEVER Romanized Bangla/Banglish (e.g. write "
            "'সবচেয়ে বড়', not 'shobcheye boro'). Only tech terms (API, SEO, Cloud, Database) stay "
            "in English. Follow your brand style, then self-review it. Remember: teach first, end "
            "naturally, never add a promotional closing or forced CTA — if the point is made, stop. "
            "Format your final answer as JSON with exactly these keys:\n"
            "{\n"
